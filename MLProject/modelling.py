...
def get_tracking_uri():
    """Get tracking URI from environment or use default SQLite."""
...
def main():
    ...
    # Check if we're running under MLflow Project
    # MLflow Project sets MLFLOW_RUN_ID and has active run already
    is_under_mlflow_project = os.getenv("MLFLOW_RUN_ID") is not None
    
    # Only start a new run if NOT under MLflow Project
    if not is_under_mlflow_project:
        mlflow.set_experiment("Salary Regression Baseline")
        mlflow.start_run(run_name="baseline_random_forest")
        should_end_run = True
    else:
        # Already have an active run from MLflow Project
    ...
    try:
        ...
        mlflow.log_artifact(str(model_path), artifact_path="backup_model")
